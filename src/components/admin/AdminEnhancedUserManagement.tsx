...
import { Users, Search, Edit, Trash2, UserPlus, DollarSign, MessageSquare, Send } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { useToast } from '@/components/ui/use-toast';

export const AdminEnhancedUserManagement = () => {
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-users-enhanced'] });
      toast({ title: "Message Sent", description: "Message has been sent to the user successfully." });
      setMessagingUser(null);
      setMessageContent({ subject: '', content: '' });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const handleSendMessage = () => {
    if (messagingUser && messageContent.subject && messageContent.content) {
  ...
  const getUnreadNotifications = (user: any) => {
    return user.user_notifications?.filter((n: any) => !n.is_read).length || 0;
  };
  ...
  return (
    <div className="space-y-6">
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        <Badge className="bg-purple-600/20 text-purple-400">
                          {getUnreadNotifications(user)} unread
                        </Badge>
                        <p className="text-xs text-gray-400">
                          {user.user_notifications?.length || 0} total notifications
                        </p>
                      </div>
                    </TableCell>
                    <TableCell>
      ...
      {messagingUser && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <Card className="bg-gray-800 border-gray-700 p-6 w-full max-w-lg mx-4">
            <h3 className="text-xl font-bold text-white mb-4">Send Message to User</h3>
            <div className="mb-4">
              <p className="text-gray-300">
                <strong>To:</strong> {messagingUser.full_name || messagingUser.username} ({messagingUser.email})
              </p>
            </div>
            <div className="space-y-4">
              <div>
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <Button
                ...
                disabled={!messageContent.subject || !messageContent.content}
                className="flex-1 bg-blue-600 hover:bg-blue-700"
              >
